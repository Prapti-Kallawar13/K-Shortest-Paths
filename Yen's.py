...
def k_shortest_paths(G, source, target, k = 1, weight = 'weight'):
    ...
    C =nx.single_source_dijkstra_path(G, source,cutoff=None,weight = 'weight')
    E=list(C[target])
...
#To remove the root nodes(here n is equal to  NODE)            
            ...
            try:
              ...
              D = nx.single_source_dijkstra_path(Gcopy, spurnode,cutoff=None,weight = 'weight')
              spurpath = list(D[target])
...
for k in range (1,16) : 
    begin = time()
    store_counts = k_shortest_paths(G, source=1, target=10, k = k, weight = 'weight')
    end = time()
    times.append(end-begin)
    
    
times = times
print("Time required:",times)
plt.show()

storage = store_counts

#Declare the lists to store the values of x and y axis
x= []
y= []
x1= []
y1= []
...
#update the list to final values  
x=x
y=y
x1=x1
y1=y1
