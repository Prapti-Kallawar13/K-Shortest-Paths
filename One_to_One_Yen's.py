...
# In spurpath Value ranges from 1 onwards
        ...
        B.remove(B[0])
    print(f"Shortest paths found for k={k}:\n",A)
...
for k in range (1,16) : 
    begin = time()
    store_counts = k_shortest_paths(G, source=1, target=95, k = k, weight = 'weight')
    end = time()
...
#Declare the lists to store values of x and y axis
x= []
y= []
x1= []
y1= []
...
#Updating the list for final values at x and y axis   
x=x
y=y
x1=x1
y1=y1
